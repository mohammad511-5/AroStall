...
export function ChatWidget({ isCartOpen }: { isCartOpen?: boolean }) {
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-40 w-[330px] rounded-2xl shadow-2xl shadow-black/60 overflow-hidden border border-white/10"
          >
            {/* ── Header ── */}
      ...
      {/* Floating button — hidden while cart is open */}
      <motion.button
        animate={{ opacity: isCartOpen ? 0 : 1, pointerEvents: isCartOpen ? 'none' : 'auto' } as any}
        transition={{ duration: 0.2 }}
        whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.93 }}
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-br from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-white rounded-full shadow-2xl shadow-green-500/40 flex items-center justify-center transition-all"
      >
        <AnimatePresence mode="wait">
          {open ? (
