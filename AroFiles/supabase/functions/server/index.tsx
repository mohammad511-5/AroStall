...
app.post('/make-server-f343f1a0/save-order', async (c) => {
  try {
    ...
    const allOrders = await kv.get('admin:all-orders') ?? [];
    allOrders.unshift(order);
    await kv.set('admin:all-orders', allOrders);
...
app.get('/make-server-f343f1a0/admin/orders', async (c) => {
  if (c.req.query('key') !== 'ARO_ADMIN_9f3k2p') return c.json({ error: 'Unauthorized' }, 401);
  const orders = await kv.get('admin:all-orders') ?? [];
  return c.json({ orders });
});

// Admin — complete order
app.post('/make-server-f343f1a0/admin/complete', async (c) => {
  if (c.req.query('key') !== 'ARO_ADMIN_9f3k2p') return c.json({ error: 'Unauthorized' }, 401);
  try {
    ...
    await kv.set(`orders:${order.userId}`, userOrders.map((o: any) =>
      o.orderId === orderId ? { ...o, status: 'delivered', deliveredAt } : o
    ));

    const allOrders = await kv.get('admin:all-orders') ?? [];
    await kv.set('admin:all-orders', allOrders.map((o: any) =>
      o.orderId === orderId ? { ...o, status: 'delivered', deliveredAt } : o
    ));
