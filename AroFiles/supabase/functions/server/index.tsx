...
// Save order + send Discord notification (called after payment submission)
app.post('/make-server-f343f1a0/save-order', async (c) => {
  try {
    const body = await c.req.json();
    const { orderId, userId, userEmail, userName, items, total, paymentMethod, txId, phone, robloxUser, discordWebhook, completeUrl } = body;
    ...
    // Send Discord notification from server (avoids CORS)
    if (discordWebhook) {
      const itemsList = items.map((i: any) => `• ${i.name} x${i.quantity} — ৳${(i.price * i.quantity).toLocaleString()}`).join('\n');
      const embed = {
        title: '🛒 New Order Received!',
        color: 0xF59E0B,
        description: `**Order ID:** \`${orderId}\`\n\n✅ [Click here to mark as DELIVERED](${completeUrl})`,
        fields: [
          { name: '👤 Customer', value: userName ?? 'Guest', inline: true },
          { name: '📧 Email', value: userEmail ?? 'N/A', inline: true },
          { name: '🎮 Roblox Username', value: `\`${robloxUser}\``, inline: true },
          { name: '💳 Payment Method', value: paymentMethod === 'bkash' ? 'bKash' : 'Nagad', inline: true },
