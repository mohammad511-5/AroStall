...
export function Header({ cartCount, onCartClick, onLoginClick, onDashboardClick, onLogoClick }: HeaderProps) {
  const { user } = useAuth();
  ...
  return (
    <header className="bg-black border-b-2 border-yellow-500 sticky top-0 z-50 shadow-lg shadow-yellow-500/10">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <button onClick={onLogoClick} className="flex items-center gap-3 hover:opacity-80 transition-opacity flex-shrink-0">
          <img
            src={logoImg}
            alt="AroStall"
