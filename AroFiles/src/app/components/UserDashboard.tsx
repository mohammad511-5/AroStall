...
import { X, User, LogOut, Package, Clock, CheckCircle } from 'lucide-react';
  ...
  async function fetchTransactions() {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-f343f1a0/transactions`,
        {
          headers: {
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {transactions.map((transaction) => (
                  <div
                    key={transaction.id}
                    className="bg-white/5 rounded-lg p-4 border border-yellow-500/10 hover:border-yellow-500/30 transition-colors"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-white/30 text-xs font-mono">{transaction.id}</p>
                      </div>
                      <div className="flex items-center gap-1 text-green-400 text-xs bg-green-500/10 border border-green-500/30 px-2 py-1 rounded">
