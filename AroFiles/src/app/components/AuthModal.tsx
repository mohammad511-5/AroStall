    ...
    <div className="flex gap-3 justify-center">
      {Array.from({ length: 6 }).map((_, i) => (
        <motion.input
          ...
          className="w-11 h-14 text-center text-xl font-bold text-white bg-white/5 border border-yellow-500/30 rounded-xl focus:outline-none focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/30 transition-all caret-yellow-400"
        />
      ))}
    </div>
  );
                  ...
                  >
                    ...
                    {screen === 'verify-signup' && (
                      <div className="space-y-6">
                        <div className="text-center">
                          ...
                          <p className="text-white/50 text-sm">We sent a 6-digit code to</p>
                    ...
                    {screen === 'forgot-email' && (
                      <form onSubmit={handleForgotSend} className="space-y-4">
                        <p className="text-white/50 text-sm pb-1">Enter your email and we'll send you a 6-digit reset code.</p>
                        <div className="relative">
                          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-yellow-500/60" />
                          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                            className={inputCls} placeholder="your@email.com" required />
