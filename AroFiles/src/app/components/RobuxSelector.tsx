...
export function RobuxSelector({ onAddToCart }: RobuxSelectorProps) {
  ...
  const rateBdt = Number(settings.robux_rate_bdt) || 850;
  const rateUsd = Number(settings.robux_rate_usd) || 6.89;
  ...
  const price = currency === 'USD'
    ? Math.round((robuxAmount / 1000) * rateUsd * 100) / 100
    : Math.round((robuxAmount / 1000) * rateBdt);
  ...
  const addToCart = () => {
    const bdtPrice = Math.round((robuxAmount / 1000) * rateBdt);
    const usdPrice = Math.round((robuxAmount / 1000) * rateUsd * 100) / 100;
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/5 rounded-xl p-6 border border-yellow-500/20">
              ...
              <div className="bg-yellow-500/10 rounded-lg px-4 py-2 border border-yellow-500/30">
                <p className="text-yellow-300 text-sm text-center">{currency === 'USD' ? `Rate: $${rateUsd} per 1,000 Robux` : `Rate: ৳${rateBdt} per 1,000 Robux`}</p>
              </div>
            </div>
            ...
            >
              ...
              {t.addToCart} — {currency === 'USD' ? `$${price.toFixed(2)}` : `৳${price.toLocaleString()}`}
            </motion.button>
          </div>
        </div>
      </motion.div>
          ...
          <>
            ...
            >
              <div className="bg-zinc-950 border border-yellow-500/40 rounded-2xl shadow-2xl shadow-yellow-500/10 overflow-hidden">
                ...
                {/* Actions */}
                <div className="px-6 py-5 border-t border-yellow-500/20 flex gap-3">
                  <button
                    onClick={() => setShowWarning(false)}
                    className="flex-1 bg-white/5 hover:bg-white/10 text-white/70 border border-white/10 py-3 px-4 rounded-xl font-semibold transition-colors"
                  >
                    Cancel
                  </button>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    onClick={addToCart}
                    className="flex-1 bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-300 hover:to-yellow-500 text-black font-bold py-3 px-4 rounded-xl transition-colors shadow-lg shadow-yellow-500/20 text-sm leading-snug"
                  >
                    I understand,<br />Add to Cart
                  </motion.button>
                </div>
              </div>
            </motion.div>
