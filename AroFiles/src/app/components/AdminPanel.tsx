...
import { X, CheckCircle, Clock, RefreshCw, Package, Truck, Lock, Plus, Trash2, Edit2, BarChart2, Search, Eye, EyeOff, Upload, ImageIcon, MessageSquare, Send, ChevronLeft } from 'lucide-react';
...
export function AdminPanel({ isOpen, onClose }: AdminPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'stats' | 'orders' | 'products' | 'messages'>('orders');
  ...
  const fetchConversations = async () => {
    const { data } = await supabase
      .from('messages')
      .select('session_id, user_name, user_email, content, created_at, sender')
      .order('created_at', { ascending: false });
    if (!data) return;
    // Group by session_id, take latest message per session
  ...
  const sendReply = async () => {
    ...
    const convo = conversations.find(c => c.session_id === activeConvo);
    await supabase.from('messages').insert({
      session_id: activeConvo,
      user_name: 'AroStall Admin',
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-4 sm:inset-8 bg-gradient-to-b from-zinc-950 to-black border border-yellow-500/30 rounded-2xl z-[60] flex flex-col shadow-2xl shadow-yellow-500/10 overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 flex-shrink-0" style={{ borderBottom: '1px solid rgba(234,179,8,0.2)' }}>
            ...
            {!authed ? (
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {/* ── Tab bar ── */}
                <div className="flex border-b border-yellow-500/20 flex-shrink-0 bg-black/40">
                  {([
                    ...
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); if (tab.id === 'messages') { fetchConversations(); setActiveConvo(null); } }}
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={stat.label} className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-yellow-500/20 hover:border-yellow-500/40 transition-colors rounded-xl p-4 text-center">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {ordersLoading ? (
                        ...
                        <motion.div key={order.orderId} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                          className={`rounded-xl border p-4 ${order.status === 'delivered' ? 'bg-green-500/5 border-green-500/25' : 'bg-zinc-900/50 border-yellow-500/30'}`}>
                    ...
                    {activeConvo ? (
                      ...
                      <>
                        <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 flex-shrink-0">
                          <button onClick={() => { setActiveConvo(null); fetchConversations(); }} className="text-white/40 hover:text-yellow-400 transition-colors">
                            <ChevronLeft className="w-5 h-5" />
                          </button>
                          <div>
                            <p className="text-white font-semibold text-sm">{conversations.find(c => c.session_id === activeConvo)?.user_name ?? 'Guest'}</p>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                          {convoMessages.map((msg: any) => (
                            <div key={msg.id} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                              <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-sm ${msg.sender === 'admin' ? 'bg-yellow-500/20 text-white rounded-br-sm' : 'bg-white/10 text-white/90 rounded-bl-sm'}`}>
                                <p className="text-xs text-white/40 mb-0.5">{msg.sender === 'admin' ? 'You' : msg.user_name}</p>
                                <p>{msg.content}</p>
                                <p className="text-xs text-white/25 mt-0.5">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="p-3 border-t border-white/10 flex gap-2 flex-shrink-0">
                          <input
                            ...
                            className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white placeholder-white/30 text-sm focus:outline-none focus:border-yellow-400"
                          />
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                            onClick={sendReply} disabled={!replyText.trim() || sendingReply}
                            className="bg-yellow-500 hover:bg-yellow-400 disabled:opacity-40 text-black p-2 rounded-xl transition-colors">
                      ...
                      <>
                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 flex-shrink-0">
                          <p className="text-white/40 text-xs">{conversations.length} conversation{conversations.length !== 1 ? 's' : ''}</p>
                          <button onClick={fetchConversations} className="text-white/40 hover:text-yellow-400 transition-colors">
                        ...
                        <div className="flex-1 overflow-y-auto">
                          {conversations.length === 0 ? (
                            ...
                            <button key={convo.session_id}
                              ...
                              <div className="w-9 h-9 rounded-full bg-yellow-500/20 border border-yellow-500/30 flex items-center justify-center flex-shrink-0 text-yellow-400 font-bold text-sm">
                                {(convo.user_name ?? 'G')[0].toUpperCase()}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center">
                                  <p className="text-white font-semibold text-sm">{convo.user_name ?? 'Guest'}</p>
                                  <p className="text-white/25 text-xs">{new Date(convo.created_at).toLocaleDateString()}</p>
                                </div>
                                <p className="text-white/40 text-xs truncate">{convo.content}</p>
                                {convo.user_email && <p className="text-white/25 text-xs">{convo.user_email}</p>}
                              </div>
                              {convo.sender === 'user' && (
