...
export function Cart({ isOpen, onClose, items, onUpdateQuantity, onRemoveItem, total, onCheckoutComplete }: CartProps) {
  ...
  const CRYPTO_WALLETS = [
    { id: 'usdt_bep20', name: 'USDT (BEP20)', symbol: 'USDT', address: settings.wallet_usdt_bep20, color: 'bg-green-500' },
    { id: 'usdt_erc20', name: 'USDT (ERC20)', symbol: 'USDT', address: settings.wallet_usdt_erc20, color: 'bg-blue-500' },
    { id: 'usdt_trc20', name: 'USDT (TRC20)', symbol: 'USDT', address: settings.wallet_usdt_trc20, color: 'bg-red-500' },
    { id: 'btc', name: 'Bitcoin (BTC)', symbol: 'BTC', address: settings.wallet_btc, color: 'bg-orange-500' },
    { id: 'ltc', name: 'Litecoin (LTC)', symbol: 'LTC', address: settings.wallet_ltc, color: 'bg-gray-400' },
    { id: 'eth_erc20', name: 'Ethereum (ETH)', symbol: 'ETH', address: settings.wallet_eth, color: 'bg-purple-500' },
  ];

  const nagadEnabled = settings.nagad_enabled === 'true';
  ...
  const hasRobux = items.some(i => i.category === 'ROBUX');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error: dbError } = await supabase.from('orders').insert({
        ...
        items,
        total,
        payment_method: currency === 'USD' ? (selectedWallet?.name ?? 'crypto') : method,
        tx_id: txId,
        phone,
          ...
          >
            ...
            <div className="p-5 border-b border-yellow-500/20 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-bold text-white">
                  ...
                  {step === 'payment' && (currency === 'USD'
                    ? `Pay with ${selectedWallet?.name ?? 'Crypto'}`
                    : method === 'bkash' ? 'Pay via bKash' : 'Pay via Nagad')}
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 'confirm' && (
                  <motion.div key="confirm" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.25 }} className="p-5 space-y-5">
                    ...
                    {/* Checkbox 1 — only for Robux purchases */}
                    {hasRobux && (
                    <motion.button
                      whileTap={{ scale: 0.98 }}
                    ...
                    {/* Checkbox 3 — only for LIMITEDS items */}
                    {hasLimiteds && (
                      <motion.button
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        ...
                        className={`w-full text-left flex items-start gap-4 p-4 rounded-xl border transition-all ${check3 ? 'bg-green-500/10 border-green-500/40' : 'bg-white/5 border-yellow-500/20 hover:border-yellow-500/40'}`}
                      >
                        <div className={`w-6 h-6 rounded-md border-2 flex-shrink-0 mt-0.5 flex items-center justify-center transition-all ${check3 ? 'bg-green-500 border-green-500' : 'border-yellow-500/50'}`}>
                          {check3 && <Check className="w-4 h-4 text-black" strokeWidth={3} />}
                        </div>
                        <p className="text-white/80 text-sm font-semibold leading-snug uppercase tracking-wide">
                          {t.confirmLimited}
                        </p>
                    ...
                    <motion.button
                      ...
                      onClick={() => { if ((!hasRobux || check1) && (!hasHighRobux || check2) && (!hasLimiteds || check3)) setStep('method'); }}
                      disabled={(hasRobux && !check1) || (hasHighRobux && !check2) || (hasLimiteds && !check3)}
                    ...
                    {currency === 'USD' ? (
                      CRYPTO_WALLETS.map(wallet => (
                        <motion.button
                          ...
                          onClick={() => { setCryptoWallet(wallet.id); setStep('payment'); }}
                        ...
                        >
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm text-white ${wallet.color}`}>
                            {wallet.symbol}
                          </div>
                          <div className="text-left min-w-0">
                            <p className="text-white font-bold">{wallet.name}</p>
                            <p className="text-white/30 text-xs truncate">{wallet.address.slice(0, 20)}...</p>
                          </div>
                          <div className="ml-auto text-yellow-400/50 flex-shrink-0">→</div>
                        </motion.button>
                      ))
                    ) : (
                      <>
                        <motion.button
                          ...
                          className="w-full bg-white/5 hover:bg-white/10 border border-yellow-500/20 hover:border-yellow-500/50 rounded-xl p-5 flex items-center gap-4 transition-all"
                        >
                          <div className="w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg bg-pink-600 text-white">bK</div>
                          <div className="text-left">
                            <p className="text-white font-bold">bKash</p>
                            <p className="text-white/40 text-xs">Send to: {settings.bkash_number}</p>
                          </div>
                        ...
                        <div className="relative">
                          <motion.button
                            key="nagad"
                            whileHover={nagadEnabled ? { scale: 1.02 } : {}}
                            whileTap={nagadEnabled ? { scale: 0.98 } : {}}
                            onClick={() => nagadEnabled ? selectMethod('nagad') : undefined}
                            className={`w-full bg-white/5 border border-yellow-500/20 rounded-xl p-5 flex items-center gap-4 transition-all ${nagadEnabled ? 'hover:bg-white/10 hover:border-yellow-500/50 cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
                          >
                          ...
                          {!nagadEnabled && <div className="absolute top-2 right-2 bg-yellow-500 text-black text-xs font-black px-2 py-0.5 rounded-full">COMING SOON</div>}
                        </div>
                      </>
                    )}
                  </motion.div>
                ...
                {/* ── PAYMENT FORM ── */}
                {step === 'payment' && (currency === 'USD' ? !!selectedWallet : !!method) && (
                    ...
                    <div className="bg-yellow-500/5 border border-yellow-500/30 rounded-xl p-5 space-y-4">
                      <div className="text-center">
                        ...
                        <p className="text-4xl font-black text-yellow-400">
                          {currency === 'USD' ? `$${totalUsd.toFixed(2)}` : `৳${total.toLocaleString()}`}
                        </p>
                      </div>
                      <div className="h-px bg-yellow-500/20" />
                      {currency === 'USD' && selectedWallet ? (
                        <div>
                          <p className="text-white/50 text-xs uppercase tracking-wider mb-2">{selectedWallet.name} Address</p>
                          <div className="flex items-center gap-3 bg-white/5 rounded-lg px-4 py-3 border border-yellow-500/20">
                            <span className="text-white font-mono text-xs flex-1 break-all">{selectedWallet.address}</span>
                            <motion.button
                              whileTap={{ scale: 0.9 }}
                              onClick={() => copyCryptoAddress(selectedWallet.address)}
                              className={`flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded-lg transition-all flex-shrink-0 ${copiedCrypto ? 'bg-green-500/20 text-green-400 border border-green-500/40' : 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/40 hover:bg-yellow-500/30'}`}
                            >
                              {copiedCrypto ? <><Check className="w-3.5 h-3.5" />Copied!</> : <><Copy className="w-3.5 h-3.5" />Copy</>}
                            </motion.button>
                          </div>
                        </div>
                      ) : (
                    ...
                    <div className="bg-white/5 rounded-xl p-4 border border-white/10 text-xs text-white/50 space-y-1">
                      {currency === 'USD' && selectedWallet ? (
                        <>
                          ...
                          <p>2. Send exactly <span className="text-yellow-400 font-semibold">${totalUsd.toFixed(2)}</span> to the wallet address above</p>
                          <p>3. Copy your <span className="text-white/80">Transaction Hash/ID</span> from the confirmation</p>
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="text-white/50 text-xs uppercase tracking-wider mb-1.5 block">
                          {currency === 'USD' ? 'Transaction Hash/ID' : 'Transaction ID'}
                        </label>
                        <input value={txId} onChange={e => setTxId(e.target.value)} className={inputCls} placeholder={currency === 'USD' ? 'e.g. 0xabc123...' : 'e.g. 8N7A2KQX1P'} />
                      </div>
                      <div>
                        <label className="text-white/50 text-xs uppercase tracking-wider mb-1.5 block">
                          {currency === 'USD' ? 'Your Email or Contact' : 'Phone Number (sent from)'}
                        </label>
                        <input value={phone} onChange={e => setPhone(e.target.value)} className={inputCls} placeholder={currency === 'USD' ? 'e.g. user@email.com' : 'e.g. 01712345678'} />
                      </div>
                      <div>
            ...
            {step === 'cart' && items.length > 0 && (
              <div className="p-5 border-t border-yellow-500/20 bg-white/5 flex-shrink-0">
                ...
                <motion.button
                  whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                  onClick={() => { if (!user) return; setCheck1(false); setCheck2(false); setCheck3(false); setStep('confirm'); }}
                  disabled={!user}
