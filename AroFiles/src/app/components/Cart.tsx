...
const DISCORD_WEBHOOK = 'https://discord.com/api/webhooks/1521938520753701017/PCutGpFubjhNgdNPj-Zyx6gs_EOyz6h1rHEes0xf9tlgtsOMSjnOV78GG0C_VAHurh15';
const ADMIN_KEY = 'ARO_ADMIN_9f3k2p';
...
export function Cart({ isOpen, onClose, items, onUpdateQuantity, onRemoveItem, total, onCheckoutComplete }: CartProps) {
  ...
  const handleSubmit = async () => {
    ...
    const completeUrl = `https://${projectId}.supabase.co/functions/v1/make-server-f343f1a0/complete-order?id=${orderId}&key=${ADMIN_KEY}`;
    const itemsList = items.map(i => `• ${i.name} x${i.quantity} — ৳${(i.price * i.quantity).toLocaleString()}`).join('\n');

    const embed = {
      title: '🛒 New Order Received!',
      color: 0xF59E0B,
      description: `**Order ID:** \`${orderId}\`\n\n✅ [Click here to mark as DELIVERED](${completeUrl})`,
      fields: [
        ...
        { name: '💳 Payment Method', value: method === 'bkash' ? 'bKash' : 'Nagad', inline: true },
        { name: '📱 Sent From', value: phone, inline: true },
        { name: '🔖 Transaction ID', value: `\`${txId}\``, inline: true },
        { name: '🛍️ Items', value: itemsList },
        { name: '💰 Total Paid', value: `৳${total.toLocaleString()}`, inline: true },
      ],
      timestamp: new Date().toISOString(),
      footer: { text: 'AroStall Payment System' },
    };

    try {
      // Save order to backend
      await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-f343f1a0/save-order`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            orderId, userId: user?.id, userEmail: user?.email,
            userName: user?.name, items, total, paymentMethod: method,
            txId, phone, robloxUser,
          }),
        }
      );
